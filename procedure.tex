...
However, \prog{NWChem} can compute the values only up to a minimum distance to the nuclei.
...
In the program \prog{FreeWilly} the finite elements is obtained from a set of points via Delaunay triangulation %TODO (see \ref{app:delaunay} for details) 
using the library \prog{tetgen} \cite{tetgen}.
...
The left panel in Figure \ref{fig:Benzotrsh} shows the functional \eq{eq:J_ao} for benzene.
