...
The electrostatic potential used in this work is obtained with a modified version of \prog{NWChem} \cite{nwchem} where the points, at which this potential should be computed can be set by the user.
...
In the developed protocol, the DO is computed with the in hous code \prog{DYSON} \cite{MAgg}, having the coefficients for the primitive basis functions as its output.
From these primitives, the values of the DO at all quadrature points are computed with a self-written function in \prog{FreeWilly}.
