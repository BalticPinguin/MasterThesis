...
Setting these points to the quadrature points of the integration scheme requires some extra effort but has the great advantage that the interpolation can be restricted to the region close to the nuclei, where \prog{NWChem} can not compute the ESP explicitly.
An interpolation of the data on the complete domain in \prog{FreeWilly} \cite{FreeWilly} would require an interpolation-scheme for scattered data such as RBF interpolation \cite{rbfSE,rbfInterpol,rbfSurf}, inverse distance interpolation \cite{adapt_idw,idw} or similar methods \cite{CompInterp,idw_krieging,interpol}, but these methods give, if the parameters are not chosen accurately, spurious oscillations between the quadrature points and thus do not result in a smooth ESP.
...
The wave functions of the initial and final states are computed with the \prog{NWChem}-code which is modified to produce more verbose output.
...
The \prog{FreeWilly}-code supports supports several closely related setup-schemes of which the most important are described in the following.

\subsection{Setup of the Grid}
\label{sec:grid}
%The most crucial part in a FEM simulation is the mesh being used. 
In the program \prog{FreeWilly} the finite elements is obtained from a set of points via Delaunay triangulation (see \ref{app:delaunay} for details) using the library \prog{tetgen} \cite{tetgen}.
...
\end{equation}
where $l$ is a parameter that determines the distance between the most inner spheres.
Using this function, the smallest spheres ($i<<N$) are scaled linearly with distances $r_\text{max}/N$ whereas the distance of the largest spheres depends non-trivially on the parameters but can become very large if $l$ is small whereas large $l$ lead to almost uniform distribution of spheres.
...
\end{equation}
is suggested which has two degrees of freedom $l$ and $p\geq 1$ where the condition $N<\frac{r_\text{max}}{l}$ needs to be fulfilled for a well-behaved mapping.
Here $l$ is the asymptotic distance between two spheres whereas the distance of inner spheres can be tuned by the power $p+1$ so that the inner spheres can be tuned to become very dense.
